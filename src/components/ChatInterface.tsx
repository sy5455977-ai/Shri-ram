...
export default function ChatInterface({ conversationId, onConversationCreated, performanceMode }: ChatInterfaceProps) {
  ...
  // Use a ref to store messages for stable callbacks to prevent O(N^2) re-renders
  const messagesRef = useRef<Message[]>(messages);
  useEffect(() => {
    messagesRef.current = messages;
  }, [messages]);

  // Ref to always point to the latest handleSend for handleRegenerate stability
  ...
  const handleRegenerate = React.useCallback(async () => {
    ...
    // O(1) space backward loop to find the last user message
  ...
  }, [input, selectedImage, conversationId, deepReasoning, onConversationCreated]);
